...
public class SecurityGroupsClientTest {
	...
	public static void setup() {
		DevCiCredentialsProvider devCiCred = new DevCiCredentialsProvider();
		AwsConnectionFactory factory = new AwsConnectionFactory(
				devCiCred.getCredentials().getAWSAccessKeyId(),
				devCiCred.getCredentials().getAWSSecretKey());
		factory.setServiceEndpoint("ec2.us-east-1.amazonaws.com");

		service = new AwsServiceInterface();

		((AwsServiceInterface) service).setConnectionFactory(factory);

	}

	@Test
	public void createGroups() {
		SecurityGroupsClient client = new SecurityGroupsClient(service);
		assertNotNull(client);
		client.createTmsGroups();
	}
