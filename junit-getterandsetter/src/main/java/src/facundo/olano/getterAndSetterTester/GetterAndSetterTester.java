 ...
 *************************************************************************/
package src.facundo.olano.getterAndSetterTester;
...
/**
 * Class to test using JUnit the getters and setters of a class. The basic usage
 * is to test the methods (matching beans naming conventions) of a given object,
 * using {@code testInstance(instance)}, or a given class, with
 * {@code testClass(clazz)}. As it works by instantiating the classes to test,
 * and those to get and set the fields, if a class is too complex to
 * instantiate, functinoality is provided to ignore specific fields or store
 * default instances of the classes to use.
 *
...
public class GetterAndSetterTester {
	...
	/**
	 * Adds a map of instances to use by default by the tester when an instance
	 * of a given class is needed. Every time an instance of a class is needed
	 * to be used on a setter or to construct a class, the class will be looked
	 * in the map and the value object will be used as instance.
	 *
	 * @param instances
	 *            the map with classes and its instances to use.
	 */
	...
	/**
	 * Test the accessors of the given instance. Only instantiation through
	 * default constructors or primitive types is supported, therefore the
	 * tested class accessible fields are expected to be recursively traced to
	 * classes that can be instantiated that way, otherwise the test will throw
	 * stack overflows while trying to instantiate classes. If complicated
	 * instantiations are involved, it's recommended to use the
	 * addDefaultInstances method.
	 *
	 * @param instance
	...
	public void testInstance(final Object instance) {
		...
		for (Iterator<Method> iterator = methods.iterator(); iterator.hasNext();) {
			...
			try {
				...
				if (classAR != null && classAR.length >0) {
					Object[] arr = new Object[classAR.length];
					for (int i = 0; i < classAR.length; i++) {
						arr[0] = defaultInstances.get(classAR[0]);
					}
	 ...
	 * instantiated. Only instantiation through default constructors or
	 * primitive types is supported, therefore the tested class constructor
	 * arguments and accessible fields are expected to be recursively traced to
	 * classes that can be instantiated that way, otherwise the test will throw
	 * stack overflows while trying to instantiate classes. If complicated
	 * instantiations are involved, it's recommended to use the
	 * addDefaultInstances method.
	 ...
	 *
	 * @param field
	 *            the field to get the setter from.
	...
	/**
	 * Returns the getter of the field or null if there is none. Both
	 * get(FieldName) and is(FieldName) method will be checked. If the method
	 * return type is different from field type, the method will be ignored.
	 *
	 * @param field
	 *            the field to get the getters from.
	...
	private Method getGetter(final Field field) {
		...
		if (theMethod != null
				&& !theMethod.getReturnType().equals(field.getType())) {
			theMethod = null;
		}
		return theMethod;
	}
	...
	/**
	 * Returns the name of the field with its first character as a capital
	 * letter. useful to append it to a method name such as a getter or setter.
	 *
	 * @param field
	 *            the field to get the name of.
	 * @return the name.
	 */
	...
	/**
	 * Test the getter and setter method of the field, using the given instance
	 * of the class.
	 *
	 * @param field
	 *            the field to test the getter and setter.
	 * @param instance
	 *            the instance to use to test.
	 */
	private void testGetterAndSetter(final Field field, final Object instance) {
		...
		try {
			...
			TestCase.assertEquals("Failed getter and setter test of field "
					...
					+ field.getDeclaringClass().getName(), value,
	...
	/**
	 * Test the setter method of the field, using the given instance of the
	 * class.
	 *
	 * @param field
	 *            the field to test the setter.
	 * @param instance
	 *            the instance to use to test.
	 */
	private void testSetter(final Field field, final Object instance) {
		...
		try {
			...
			TestCase.assertEquals(
					"Failed setter test of field " + field.getName()
							+ " on classs "
							+ field.getDeclaringClass().getName(), value,
					field.get(instance));
	...
	/**
	 * Test the getter method of the field, using the given instance of the
	 * class.
	 *
	 * @param field
	 *            the field to test the getter.
	 * @param instance
	 *            the instance to use to test.
	 */
	private void testGetter(final Field field, final Object instance) {
		...
		try {
			...
			TestCase.assertEquals(
					"Failed getter test of field " + field.getName()
							+ " on classs "
							+ field.getDeclaringClass().getName(), value,
					getter.invoke(instance));
	...
	/**
	 * Recursive method to instantiate a given class. First the map of default
	 * instances will be checked for an instance. If not existent, then if a
	 * default (no arguments) constructor exists, it will be used, otherwise any
	 * other constructor will be tried until the class can be instantiated. The
	 * last choice will be returning null.
	 *
	 * @param aClass
	...
	/*
	 * TODO instead of using get in the map test each class using
	 * isAssignableFrom.
	 */
	...
	/**
	 * Sets a list of the names of the fields that should be ignored. All the
	 * fields in the list won't be tested.
	 *
	 * @param fields
