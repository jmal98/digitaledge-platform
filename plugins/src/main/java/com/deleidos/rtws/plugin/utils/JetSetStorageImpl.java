 ...
 */
package com.deleidos.rtws.plugin.utils;
...
public class JetSetStorageImpl implements StorageInterface {
	...
	public void storeFile(String bucketName, String fileKey, File sourceFile)
		...
		if (sourceFile == null || !sourceFile.exists()) {
			throw new IOException("Source file [" + sourceFile == null ? "null" : sourceFile.getAbsolutePath()
					+ "] cannot be found");
		}
		...
		try {
			...
			throw new IOException("MD5 hash algorithm is not supported!", e);
		} catch (S3ServiceException s3e) {
			throw new IOException("Jets3t failure in storeFile()", s3e);
		}
		finally {
			if (s3Object != null) {
				try {
	...
	private synchronized S3Service getClient()
			throws IOException {
		if (s3client == null) {
			try {
				...
				props.setProperty("httpclient.max-connections", "4");
				...
				throw new IOException("JetS3t factory failed!", s3e);
			}
		}
		return s3client;
	}
